  ...
  <div>
    <div class="home_user_header">
      <h3 class="comman-title">
        {{ slug ? "Update" : "Create" }} Platform Master
      </h3>
    </div>
          ...
          >
            ...
            >
              <small
                ...
                (Image Size in Pixel - 100x100)</small
              >
            </q-tooltip>
          </q-btn>
          <div>
...
onMounted(() => {
  if (route.name !== "edit-platformMaster") {
    // isIdle.value = true;
  }
});
...
if (route.name == "edit-platformMaster") {
  ...
  api
    .get(api.resolveApiUrl(PLATFORM_MASTER.SHOW, { id: slug.value }))
    .then((res) => {
      if (res.success == true) {
...
async function onSubmit() {
  ...
  if (route.name == "edit-platformMaster") {
    store
      .updatePlatformMaster(postData, edit_id.value)
      .then((res) => {
        if (res.success) {
          router.push({ name: "platformMaster-dashboard" });
          Loading.hide();
        } else {
    ...
    store
      .saveNewPlatformMaster(postData)
      .then((res) => {
        if (res.success) router.push({ name: "platformMaster-dashboard" });
        else {
          notification.error(res.message);
          Loading.hide();
        }
